...
export const CodeOutputCard = ({ message, onJumpToSource }: CodeOutputCardProps) => {
  ...
  const language = extractLanguage(message.content);
  const cleanedCode = cleanCode(message.content);
      ...
      {message.metadata?.model && (
        <div className="mt-3 pt-3 border-t border-gray-700 text-xs text-gray-400">
          ...
          {message.metadata.prompt && (
            <div className="mt-1 text-gray-500 italic">"{message.metadata.prompt}"</div>
          )}
        </div>
      )}
    </div>
